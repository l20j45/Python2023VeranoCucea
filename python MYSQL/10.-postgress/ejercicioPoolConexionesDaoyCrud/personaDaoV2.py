...
class PersonaDAO:
    ...
    def actualizar(cls, persona):
        with poolCursor() as cursor:
            personaBuscada = PersonaDAO.seleccionarEspecifico(
                persona.id_persona)
            log.debug(f'Persona a modificar {personaBuscada}')
            valores = (persona.nombre, persona.apellido,
                       persona.email, persona.idpersona)
            cursor.execute(cls._ACTUALIZAR, valores)
            log.debug(
                f'Persona a modificar {PersonaDAO.seleccionarEspecifico(id_persona)}')
            return cursor.rowcount
...
""" # devolver registros
    ...
    persona1 = Persona(nombre='riot',apellido='propaganda',email='rsandoval@gmail.com')
